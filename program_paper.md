...
This is an autonomous active learning loop for molecular property prediction on HMS O2, driven by an LLM agent (DeepSeek via [opencode](https://github.com/sstcloud/opencode)). **Before starting, read the full reference paper in the section below — it describes prior experimental results on this exact dataset and task.**
...
## Setup
1. **Branch**: Already on `al/may13_v2` — stay here.
2. **results.tsv**: Already initialized with baseline row.
3. **Data**: train_df.csv (10% initial labeled), pool_df.csv (90% unlabeled candidates), test_df.csv (held-out). All three are disjoint.
